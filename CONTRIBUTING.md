...
- **Dependencies:** **`au.gov.nehta:pcehr-compiled-wsdl`** at **`pcehr.wsdl.version`** = **`${project.version}`**. **Local SNAPSHOT:** **`mvn install`** in **[pcehr-compiled-wsdl-java](https://github.com/AuDigitalHealth/pcehr-compiled-wsdl-java)** (**`java-8-javax`**) before **`verify`** here.
