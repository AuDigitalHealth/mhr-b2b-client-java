...
- Maven **`au.gov.nehta:mhr-b2b-client`** **24.0.0** (Java **24** / **Jakarta**, **15** facades). The first number of the Maven version is the targeted Java SE version. Pair with **`au.gov.nehta:mhr-wsdl`** at the same coordinate.
- Sibling libs **`common-library`**, **`smi-xsp`**, and **`smi-common-utils`** at **`${project.version}`** (**24.0.0**).
...
- Javadoc: **`doclint=none`**, **`failOnWarnings=false`** (same as other Java **24** sibling lines).
