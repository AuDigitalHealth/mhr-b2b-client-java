...
Branch model: **`java-11-jakarta`** (JDK **11**, Jakarta XML Web Services, **`jaxws-rt` 4.0.4**). Compile against **`au.gov.nehta:pcehr-compiled-wsdl`** at **`${pcehr.wsdl.version}`** = **`${project.version}`** (**`1.6.5-SNAPSHOT`** during dev); install **`pcehr-compiled-wsdl-java`** (`java-11-jakarta`) first.
...
5. **LICENSE.txt**, **README.md**, **CONTRIBUTING.md**, **SECURITY.md**, **ADHA-THIRD-PARTY-SCOPE.md** current for **`java-11-jakarta`**.
